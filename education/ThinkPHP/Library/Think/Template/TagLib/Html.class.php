...
class Html extends TagLib{
    ...
    protected $tags   =  array(
		...
		'input' 	=> array('attr'=>'type,name,id,classid'), // input标签
		'textarea' 	=> array('attr'=>'name,id,class,style,width,height')
        );
    ...
    public function _editor($tag,$content) {
     ...
     //   $content    =   $tag['content'];
        $type       =   $tag['type'] ;
		$editmenu       =   !empty($tag['editmenu'])?','.$tag['editmenu']:'';
        switch(strtoupper($type)) {
            case 'CKEDITOR':
                $parseStr   =	'<!-- 编辑器调用开始 --><script type="text/javascript" src="__JS__/ckeditor/ckeditor.js"></script><script type="text/javascript" src="__JS__/ckeditor/ckfinder/ckfinder.js"></script><textarea id="'.$id.'" name="'.$name.'">'.$content.'</textarea><script type="text/javascript">var editor = CKEDITOR.replace("'.$id.'",{width:"'.$width.'",height:"'.$height.'"'.$editmenu.'});CKFinder.setupCKEditor(editor, "__JS__/ckeditor/ckfinder/");</script> <!-- 编辑器调用结束 -->';
                break;
            case 'FCKEDITOR':
            ...
            case 'KINDEDITOR':
                ...
                break;
            default :
 				$parseStr  =  '<textarea id="'.$id.'" name="'.$name.'" class="'.$class.'" style="'.$style.'" >'.$content.'</textarea>';
        }
    ...
    public function _input($tag,$content) {
		...
		$type = $tag['type'];
		$class = $tag['class'];
		$str = "<input type='".$type."' id='".$id."' name='".$name."' class='".$class."' value='".$content."' />";
		return $str;
    }
    ...
    public function _textarea($tag,$content) {
		...
		$class = $tag['class'];
		$style = $tag['style'];
		$str = '<textarea id="'.$id.'" name="'.$name.'" class="'.$class.'" style="'.$style.'">'.$content.'</textarea>';
		return $str;
    }
