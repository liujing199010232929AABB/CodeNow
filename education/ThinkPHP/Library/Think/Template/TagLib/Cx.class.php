...
class Cx extends TagLib {
    ...
    public function _foreach($tag,$content) {
        ...
        $name       =   $this->autoBuildVar($name);
        //$parseStr .= 'if( count($content)==0 ) : echo "'.$empty.'" ;';
        $parseStr   =   '<?php if(is_array('.$name.') && !empty('.$name.') ): foreach('.$name.' as $'.$key.'=>$'.$item.'): ?>';
        $parseStr  .=   $this->tpl->parse($content);
        $parseStr  .=   '<?php endforeach; else:  ?>';
		if($empty){
			$parseStr  .=   '<?php echo "<center class=\"nolist\">'.$empty.'</center>";?>';
		}
		$parseStr  .=   '<?php endif; ?>';
        if(!empty($parseStr)) {
            return $parseStr;
    ...
    public function _foreach222($tag,$content) {
        $name       =   $tag['name'];
        $item       =   $tag['item'];
		$empty      =   isset($tag['empty'])?$tag['empty']:'';
        $key        =   !empty($tag['key'])?$tag['key']:'key';
        $name       =   $this->autoBuildVar($name);
        $parseStr   =   '<?php if(is_array('.$name.') && !empty('.$name.')){ ?>';
