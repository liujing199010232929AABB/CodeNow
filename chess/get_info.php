...
$query = mysqli_query($connID,"select * from tb_room where id= '".$_GET['roomid']."'");          //检索指定游戏房间号的信息
$result=mysqli_fetch_array($query);	//将查询结果集返回到数组中
if(!@mysqli_num_rows($query))     						//如果未检索到相关记录
    die("ended");                 						//输出错误提示信息
...
mysqli_query($connID,"update tb_room set time = '".time()."' where ID = '".$_GET['roomid']."' limit 1");   						//更新记录信息
