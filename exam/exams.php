...
//*判断登录用户的权限*//
	$bccdNames = $adminDB->executeSQL("select id,title,start_exam, over_exam from tb_mockexaminations where id='".$_SESSION['exam_id']."' ", $connID);
	if($bccdNames){
		//	$after_time=strtotime("+900 seconds",strtotime($bccdNames[0]['start_exam']));						//考试开始后15分钟
		//	if($datetime<=$after_time and $datetime>=$start_time){												//考试开始后15分钟后不可以进入考场*/
...
//查询生成的试卷，根据传递的ID值。
$bccdname = $adminDB->executeSQL("select * from tb_mockexaminations where id='" . $_GET['examination_id'] . "'", $connID);
...
//echo $fraction;
    ...
    $smarty->assign('arrayRadio', $arrayRadio);				//将查询到的单选题数组赋给指定的模板变量
        ...
        if(isset($bccdname[0]['explains']) && $bccdname[0]['explains']!=""){
        ...
        if(isset($bccdname[0]['answer']) && $bccdname[0]['answer']!=""){
...
//}
        if(isset($_POST['title']) && $_POST['title']!="" && isset($_POST['pro_class']) && $_POST['pro_class']!=""){
			$name=isset($_SESSION['name'])?$_SESSION['name']:"";
			$user_id=isset($_SESSION['user_id'])?$_SESSION['user_id']:"";
            $insert=$adminDB->executeSQL("insert into tb_mockexaminations_user(title, examination_id, name, pro_class, fraction, sub_answer, sub_time,nameid) values('" . $_POST['title'] . "','".$_SESSION['exam_id']."', '" . $name . "', '". $procid ."','". $fraction ."','" . implode("*",$sub_answer) . "', '" . date("Y-m-d H:i:s") ."','".$user_id."')", $connID);
            if($insert){
                echo "<script>alert('试卷提交成功！');</script>";
                echo "<script>closeParent('search_mockexaminations.php?id=".$connID->lastInsertId()."');</script>";
...
//		 $string=str_replace("()","<span style=' line-height:22px; padding-left:2px; padding-right:2px;'><input name='fill[]' type='text'  style='width:80px; height:18px; line-height:18px; border:1px solid #DDDDDD;' id='fill[]' size='10' maxlength='50'></span>",$str);
    $string=str_replace("______","<span style=' line-height:22px; padding-left:2px; padding-right:2px;'><input name='fill[]' type='text'  style='width:80px; height:18px; line-height:18px; border:1px solid #DDDDDD;' id='fill[]' size='10' maxlength='50'></span>",$str);
