    ...
    foreach($examTypes as $type){
        if($_POST['exam_id'] == $type['english']){				//判断单选按钮是否为空
            ...
            if($_POST['exam_id'] == "checkbox"){
                ...
                $sub = isset($_POST[$type['english'].$arrayRadio[0]['id']])?$_POST[$type['english'].$arrayRadio[0]['id']]:"";					//提交答案
            }
            if($_POST['exam_id'] == "answer" || $_POST['exam_id'] == "explain"){
                $answer = trim(strip_tags($answer));
            }
            if($answer==$sub){
                $arrayRadio[0]['sub']=$sub;		//将结果存储到数组中
                $arrayRadio[0]['sounds']="CoalMine_Backstage/upfiles/sounds/right.mp3";
                $arrayRadio[0]['right_error']="<img src='images/right.png' border='0' />";		//将结果存储到数组中
                ...
                $arrayRadio[0]['sub']=$sub;		//将结果存储到数组中
                $arrayRadio[0]['sounds']="CoalMine_Backstage/upfiles/sounds/wrong.mp3";
                $arrayRadio[0]['right_error']="<img src='images/error.png' border='0' />";		//将结果存储到数组中
                $fraction=$fraction+0;
                if($_POST['exam_id'] == "answer" || $_POST['exam_id'] == "explain"){
                    $arrayRadio[0]['flag'] = 0;
                }
...
//            }
            ...
            if(isset($_SESSION['name']) && $_SESSION['name'] != ''){
                $lsql = "insert into tb_useranswer(username,questionid,answer,useranswer,type,questype,pro_class,addtime,content)values('".$_SESSION['name']."',".$pro_id.",'".$answer."','".$sub."','r','".$_POST['exam_id']."',".$_POST['type_id'].",'".date("Y-m-d H:i:s")."','".strip_tags($arrayRadio[0]['content'])."')";
                $adminDB->executeSQL($lsql, $connID);
            }else{
                echo "<script>alert('请重新登录！');window.open('random_login.php','','width=490,height=370');window.open('','_parent','');window.close();</script>";
            }
...
function calStyle($params){
    ...
    if($t == 'text'){
        if($length <= 50){
            $size = "68px;";
        }elseif($length>50 && $length<=150){
            $size = "48px;";
        }elseif($length>150 && $length<=250){
            $size = "28px;";
        }else{
            $size = "18px;";
        }
        ...
        if($length <= 102){
            $size = "68px;";
        }elseif($length>102 && $length<=205){
            $size = "48px;";
        }elseif($length>205 && $length<=405){
            $size = "28px;";
        }else{
            $size = "18px;";
        }
...
$smarty->register_function('calStyle','calStyle');

function delbr($params){
    extract($params);
    $str = str_replace("<br/>"," ",$str);
    $str = str_replace("A","<br/>A",$str);
    if(strpos($str,"、<br/>A") !== false){
        $str = str_replace("、<br/>A","、A",$str);
    }
    return $str;
}
$smarty->register_function('delbr','delbr');
//指定模板页
$smarty->display('Random_Answerspp.phtml');
