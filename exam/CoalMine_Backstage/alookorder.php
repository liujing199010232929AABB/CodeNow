...
$titles = $adminDB->executeSQL("select id, title, exam_user from tb_examination where id='".$examination_id."'", $connID);		//从试卷数据表中查询出指定的数据
$smarty->assign('titles', $titles);

if(isset($_POST['que_submit']) && $_POST['que_submit']!=""){
	$array_id=explode("@",$titles[0]['exam_user']);
	$array_user=array();
	$admission = $adminDB->executeSQL("select admission from tb_examination_user where examination_id='".$examination_id."' ", $connID);	//从数据库中查询出考题数据
	for($n=0; $n<count($admission); $n++){
		$user_id = $adminDB->executeSQL("select id from tb_user where admission='".$admission[$n]['admission']."' ", $connID);	//查询出指定人员的考试数据
		$array_user[]=$user_id[0]['id'];
	}
	$result = array_diff($array_id, $array_user);		//获取缺考人员的数据
	if(count($result)>0){
		$smarty->assign('res', "T");
		$array_users=array();
		for($n=0; $n<count($result); $n++){
			$user_id = $adminDB->executeSQL("select name from tb_user where id='".$result[$n]."' ", $connID);	//查询出指定人员的考试数据
			$array_users[]=$user_id[0]['name'];
		}
		$smarty->assign('result', $array_users);
	}
}
...
if (isset($_POST['submit']) && $_POST['submit']) {
    ...
    if($_POST['admission']!=''){
        $where.=" and admission like '%".$_POST['admission']."%' ";
    }   
    if($_POST['pro_class']!=''){
	...
	$sql = "select * from tb_examination_user ".$where;
...
if (isset($_POST['no_submit']) && $_POST['no_submit'] != '' ) {
    $search=$adminDB->executeSQL("select * from tb_examination_user where examination_id='".$_POST['examination_id']."' and fraction < '" . $_POST['no'] . "' order by fraction desc ", $connID);
	if ($search) {
		$smarty->assign("search_count",count($search));														//将查询结果数量赋给模板变量	
		$smarty->assign("search_res",$search);														//将考题内容赋给模板变量	
    	$smarty->assign('search', 't');	
...
if($_SESSION['permissions']==3){
    if ($adminDB->executeSQL("delete from tb_examination_user where id='" . $_GET['id'] . "'", $connID)) {
        echo '<script>alert("试卷删除成功！"); window.location.href="alookorder.php?examination_id='.$_GET['examination_id'].'&big_type='.$_GET['big_type'].'&small_type='.$_GET['small_type'].'";</script>';
    } else {
        echo '<script>alert("试卷删除失败！")</script>';
    }

}else{
	echo '<script>alert("您不是超级管理员，不具备删除试卷功能！"); window.location.href="alookorder.php?examination_id='.$_GET['examination_id'].'&big_type='.$_GET['big_type'].'&small_type='.$_GET['small_type'].'";</script>';
}
...
$bcjyzs = $pageDB->pageData("select * from tb_examination_user where examination_id='".$_GET['examination_id']."' order by sub_time desc", $connID, 20, $page, 10);
...
$smarty->display('alookorder.phtml');