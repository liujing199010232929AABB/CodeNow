...
//        //生成随机简答题
        /*if(count($answers) && $typedisArr['answer'] && $typedisArr['answer'] != '' && $typedisArr['answer'] != 0){
...
if (isset($_POST['title']) && $_POST['title'] != '') {
    if (! $adminDB->executeSQL("insert into tb_mockexaminations(title, pro_class,start_exam, over_exam, radio, checkbox, fill, judgment, dates,templateid) values('" . $_POST['title'] . "', '" . $_POST['pro_class'] . "', '".$start_exam."','".$over_exam."','" . $radio . "','" . $checkbox . "','" . $fill . "','" . $judgment . "', '". date("Y-m-d ") ."',".$_POST['template']. ")", $connID)) {
        echo "<script>alert('模拟试卷添加失败！');</script>";
    } else {
        echo "<script>alert('模拟试卷添加成功！');window.location.href='aeditbccdnames.php?big_type=试卷管理 & small_type=管理模拟考核试卷'</script>";
...
//
$smarty->display('aaddbccdnames.phtml');
